...
class BGModel(BaseModel):
    ...
    def forward(self, inps, depths, depth_masks, return_orig_size=False):
        ...
        # downsample the inps
        d_h, d_w = h // 8, w // 8
        downsampled_inps = F.interpolate(inps, size=(d_h, d_w), mode="bilinear", align_corners=True)
        preds = self.model(downsampled_inps, final_size=self.final_size, return_orig_size=return_orig_size)

...
class ConvGRUCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, bias):
        ...
        self.conv2 = nn.Conv2d(in_channels=input_dim + hidden_dim,
    ...
    def forward(self, input_tensor, h_cur):
        ...
        combined_conv1 = self.conv1(combined)
        cc_r, cc_z = torch.split(combined_conv1, self.hidden_dim, dim=1)

        if input_tensor.size(0) != 1:
            cc_r = torch.sigmoid(cc_r)
        cc_rh = cc_r * h_cur
        combined2 = torch.cat([input_tensor, cc_rh], dim=1)
        ombined_conv2 = self.conv2(combined2)
        cc_g = torch.split(ombined_conv2, self.hidden_dim, dim=1)

        if input_tensor.size(0) == 0:
            z = cc_z
            g = cc_g
            h_next = z * g + (1-z) * h_cur
        else:
